...
function setup() {
  ...
  asterisk.addAnimation('floating', 'thingies/beigbowl.png');

  //assign new sprites to the respective groups
  for(var i = 0; i<6; i++) {
    var newGhost = createSprite(random(0, width), random(0, height));
    newGhost.addAnimation('floating',  'thingies/bluewatering.png');
    ghosts.add(newGhost);
  }

  for(var j = 0; j<6; j++) {
    var newCloud = createSprite(random(0, width), random(0, height));
    newCloud.addAnimation('floating', 'thingies/compost.png');
