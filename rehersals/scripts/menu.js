const gifVerse = [
  ...
  'gifwave',
  'gifducky',
]; // for each of these instances, a single pixel element will be created.
...
const createPixel = () => {
  ...
  pixelContainer.style.left =
    Math.random(window.innerWidth) * ((window.innerWidth / 4) * 3) + 'px';
  pixelContainer.style.top =
    Math.random(window.innerHeight) * (window.innerHeight / 4) + 'px';
...
const revealPixelPortal = () => {
  ...
  for (let m = 0; m < pixelPortal.length - 1; m++) {
    // the last pixel has no accompanying class on rollover - it's purely there for the nudge
...
const nudgePixels = () => {
  ...
  anchorPixel.addEventListener('mouseover', function (event) {
    let currentX = window
      .getComputedStyle(pixelContainer, null)
    ...
    newNum + nudgeAmtCalc < window.innerWidth - nudgeAmtCalc * 2
      ? pixelContainer.style.setProperty('left', newNum + 5 + 'px')
      : pixelContainer.style.setProperty('left', 5 + nudgeAmtCalc + 'px');
    // pixelContainer.style.setProperty('top', currentY + 'px');
  });
  pixelPatches[pixelPatches.length - 1].addEventListener('mouseover', function (
    event
  ) {
    let currentX = window
      .getComputedStyle(pixelContainer, null)
    ...
    newNum - 5 < 1
      ? pixelContainer.style.setProperty(
          'left',
          window.innerWidth - nudgeAmtCalc * 6 + 'px'
        )
      : pixelContainer.style.setProperty('left', newNum - 5 + 'px');
  });
  ...
  anchorPixel.addEventListener('click', function (event) {
    ...
    const newNum = parseInt(currentX.replace(/[^0-9.]+/, ''));
    // to make the test that the position doesn't exceed the window size, i need it to remain and inT - leading to the not as elegant passing of a string concatination in the setProperty
    newNum + 5 < window.innerWidth
      ? pixelContainer.style.setProperty('left', newNum + 15 + 'px')
      : pixelContainer.style.setProperty('left', nudgeAmtCalc + 'px');
    // pixelContainer.style.setProperty('top', currentY + 'px');
  });
  pixelPatches[pixelPatches.length - 1].addEventListener('click', function (
    event
  ) {
    let currentX = window
      .getComputedStyle(pixelContainer, null)
    ...
    const newNum = parseInt(currentX.replace(/[^0-9.]+/, ''));
    newNum - 5 < 1
      ? pixelContainer.style.setProperty('left', window.innerWidth - 25 + 'px')
      : pixelContainer.style.setProperty('left', newNum - 15 + 'px');
  });
};
